...
#define DEFINE_VECTOR_CONSTRUCTOR(T) \
    VECTOR_REF(T) VECTOR_METHOD(T, constructor)(int n) { \
        \
        VECTOR_REF(T) const this = (VECTOR_REF(T))malloc(sizeof(struct VECTOR_TYPE(T))); \
        \
        int capacity = 1; \
        while (capacity <= n) \
            capacity *= 2; \
    ...
    void VECTOR_METHOD(T, insert)(VECTOR_REF(T) const this, T value, int index) { \
        if (index >= this->capacity) { \
            ...
            while (capacity <= index) \
                capacity *= 2; \
                \
            VECTOR_METHOD(T, resize)(this, capacity); \
        } \
        \
        if (index >= this->size) { \
            this->size = index + 1; \
...
#define DEFINE_VECTOR_PUSH_BACK(T) \
    void VECTOR_METHOD(T, push_back)(VECTOR_REF(T) const this, T value) { \
        VECTOR_METHOD(T, insert)(this, value, this->size); \
    }
